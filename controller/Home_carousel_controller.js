// controller.js
const carouselModel = require('../model/Home_carousel_model');
const upload = require('../multer');

// Express middleware to handle file uploads
const postCarousel = (req, res) => {
  const newItem = new carouselModel({
    name: req.body.name,
...
const getCarousel = async (req, res) => {
  try {
    const items = await carouselModel.find({}).exec();
...
const updateCarousel = async (req, res) => {
  try {
    ...
    // Perform the update
    const updatedData = await carouselModel.findByIdAndUpdate(id, updateFields, { new: true });
...
const deleteCarousel = async (req, res) => {
  try {
    const { id } = req.params;
    const deletedItem = await carouselModel.findByIdAndDelete(id);
...
module.exports = { postCarousel, getCarousel, updateCarousel, deleteCarousel };
