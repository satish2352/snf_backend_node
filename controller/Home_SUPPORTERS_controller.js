// controller.js
const SUPPORTERSModel = require('../model/Home_SUPPORTERS_model');
const upload = require('../multer');

// Express middleware to handle file uploads
const postSupporters = (req, res) => {
  const newItem = new SUPPORTERSModel({
    namesup: req.body.namesup,
...
const getSupporters = async (req, res) => {
  try {
    const items = await SUPPORTERSModel.find({}).exec();
...
// const updateSupporters = async (req, res) => {
//   try {
//     const { id } = req.params;
//     // Find the existing item
//     const existingItem = await SUPPORTERSModel.findById(id);
...
//   try {
//     const { id } = req.params;
//     const updatedData = await SUPPORTERSModel.findByIdAndUpdate(id, {
//       name : req.body.name,
...
const updateSupporters = async (req, res) => {
  try {
    ...
    // Perform the update
    const updatedData = await SUPPORTERSModel.findByIdAndUpdate(id, updateFields, { new: true });
...
const deleteSupporters = async (req, res) => {
  try {
    const { id } = req.params;
    const deletedItem = await SUPPORTERSModel.findByIdAndDelete(id);
...
module.exports = { postSupporters, getSupporters, updateSupporters, deleteSupporters };
